...
export function ListingForm({ productId }: { productId?: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setExistingPaths(data.media_urls ?? []);
      setLoading(false);
    })();
    return () => {
      active = false;
  ...
  useEffect(() => {
    ...
    let active = true;
    resolveMediaUrls(existingPaths).then((urls) => {
      if (!active) return;
      const map: Record<string, string> = {};
      existingPaths.forEach((p, i) => {
  ...
  if (loading) {
    return <div className="px-4 pt-6 text-sm text-muted-foreground">Loading listing…</div>;
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 px-4 pt-4">
      <div className="surface-card p-3.5">
        ...
        {existingPaths.length > 0 ? (
          <div className="mt-2 flex gap-2 overflow-x-auto [scrollbar-width:none]">
            {existingPaths.map((p) => (
              <div key={p} className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-muted">
                {previews[p] && !isVideoPath(p) ? (
                  <img src={previews[p]} alt="Listing media" className="h-full w-full object-cover" />
                ) : (
                ...
                <button
                  ...
                  onClick={() => setExistingPaths((prev) => prev.filter((x) => x !== p))}
      ...
      <Field
        ...
        placeholder="e.g. New Zealand White doe"
      />
