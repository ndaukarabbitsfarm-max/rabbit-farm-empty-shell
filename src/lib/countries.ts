...
export const COUNTRIES: Country[] = RAW.split(",").map((entry) => {
  const [iso, dial, ...rest] = entry.split(":");
  return { iso, dial: `+${dial}`, name: rest.join(":"), flag: toFlag(iso) };
});

/** East African markets surface first for this marketplace. */
const PRIORITY = ["TZ", "KE", "UG", "RW", "BI", "CD", "ZM", "MW", "ZA"];

export const COUNTRIES_SORTED: Country[] = [
  ...PRIORITY.map((iso) => COUNTRIES.find((c) => c.iso === iso)!).filter(Boolean),
  ...COUNTRIES.filter((c) => !PRIORITY.includes(c.iso)).sort((a, b) => a.name.localeCompare(b.name)),
];

export const DEFAULT_COUNTRY = COUNTRIES.find((c) => c.iso === "TZ")!;
